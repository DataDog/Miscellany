...
def pull_dashboards():
    ...
    for dashboard in dashboards["dashboards"]:
        ...
        print(
            "Pulling dashboard: {} with id: {}, writing to file: {}".format(
                dashboard["title"].encode("utf8"), dashboard["id"], path
            )
        )
...
def pull_monitors(tag):
    ...
    for monitor in monitors:
        if monitor["type"] == "synthetics alert":
            print(
                'Skipping "{}" as this is a monitor belonging to a synthetic test. Synthetic monitors will be '
                'automatically re-created when you push synthetic tests.'.format(
                    monitor["name"].encode("utf8")
                )
            )
            continue
        all_tags_found = True
        for tag in tags:
            if not tag in monitor["tags"]:
                ...
                print(
                    'Tag: {} not found in monitor: "{}" with tags {}'.format(
                        tag, monitor["name"].encode("utf8"), monitor["tags"]
                    )
                )
                break
        ...
        if not all_tags_found:
            print(
                'Skipping "{}" because its tags do not match the filter.'.format(
                    monitor["name"].encode("utf8")
                )
            )
        ...
        if all_tags_found:
            ...
            print(
                'Pulling monitor: "{}" with id: {}, writing to file: {}'.format(
                    new_monitor["name"].encode("utf8"), new_monitor["id"], path
                )
            )
...
def pull_users():
    ...
    for user in users["users"]:
        if not user["disabled"]:  # don't pull disabled users
            ...
            print(
                "Pulling user: {} with role: {}, writing to file: {}".format(
                    user["handle"].encode("utf8"), user["access_role"], path
                )
            )
...
def pull_synthetics_api_tests(options, tag):
    ...
    r = requests.get(
        "{}api/v1/synthetics/tests".format(
            options["api_host"]
        ), headers=headers
    )
    synthetics = r.json()
        ...
        if synthetic["type"] == "api":
            ...
            for tag in tags:
                if not tag in synthetic["tags"]:
                    all_tags_found = "false"
                    print(
                        'Tag: {} not found in synthetic: "{}" with tags {}'.format(
                            tag, synthetic["name"].encode("utf8"), synthetic["tags"]
                        )
                    )
                    break
            ...
            if all_tags_found == "true":
                ...
                path = _json_to_file(
                    "synthetics_api_tests", synthetic["public_id"], json_data
                )
                print(
                    "Pulling: {} and writing to file: {}".format(
                        synthetic["name"].encode("utf8"), path
                    )
                )
...
def pull_synthetics_browser_tests(options, tag):
    ...
    r = requests.get(
        "{}api/v1/synthetics/tests".format(options["api_host"]), headers=headers
    )
    synthetics = r.json()
    for synthetic in synthetics["tests"]:
        if synthetic["type"] == "browser":
            ...
            if all_tags_found == "true":
                count = count + 1
                json_data = requests.get(
                    "{}api/v1/synthetics/tests/browser/{}".format(
                        options["api_host"], synthetic["public_id"]
                    ),
                    headers=headers,
                ).json()
                path = _json_to_file(
                    "synthetics_browser_tests", synthetic["public_id"], json_data
                )
                print(
                    "Pulling: {} and writing to file: {}".format(
                        synthetic["name"].encode("utf8"), path
...
def pull_awsaccounts(options):
    ...
    r = requests.get(
        "{}api/v1/integration/aws".format(options["api_host"]), headers=headers
    )
    awsaccounts = r.json()
...
def pull_logpipelines(options):
    ...
    r = requests.get(
        "{}api/v1/logs/config/pipelines".format(options["api_host"]), headers=headers
    )
    rJSON = r.json()
    for item in rJSON:
        count = count + 1
...
def pull_notebooks(options):
    ...
    r = requests.get("{}api/v1/notebook".format(options["api_host"]), headers=headers)
    notebooks = r.json()
    if "errors" in notebooks:  # check if feature flag is enabled in this organisation
        if "You do not have permission" in notebooks["errors"][0]:
            exit(
                "Notebooks API (notebooks_api) feature flag is not enabled on this Datadog organisation. help@datadoghq.com for more information."
            )
...
def pull_slos(options):
    ...
    count = 0
    r = requests.get("{}api/v1/slo".format(options["api_host"]), headers=headers)
    slos = r.json()
...
def push_monitors():
    ...
    for monitor in monitors:
        with open(monitor) as f:
            ...
            if data["type"] == "composite":
                ...
                print(
                    "Pushing composite monitors:",
                    data["id"],
                    data["name"].encode("utf8"),
                    " with query ",
                    new_query.encode("utf8"),
                )
    ...
    if count > 0:
        print(
            "Pushed '{}' monitors in muted status, navigate to Monitors -> Manage downtime to unmute.".format(
                count
            )
        )
    if err_count > 0:
...
def push_synthetics_api_tests(options):
    ...
    for synthetic in synthetics:
        with open(synthetic) as f:
            ...
            if not arguments["--dry-run"]:
                r = requests.post(
                    "{}api/v1/synthetics/tests".format(options["api_host"]),
                    headers=headers,
                    json=data,
                )
...
def push_awsaccounts(options):
    ...
    if not awsaccounts:
        exit(
            "No awsaccounts are locally available. Consider pulling awsaccounts first."
        )
    ...
    for awsaccount in awsaccounts:
        with open(awsaccount) as f:
            ...
            if not arguments["--dry-run"]:
                r = requests.post(
                    "{}api/v1/integration/aws".format(options["api_host"]),
                    headers=headers,
                    json=data,
                )
    ...
    print(
        "You can now use the json files in the awsaccounts.out folder to automate the AWS External ID onboarding using AWS APIs."
    )
...
def push_logpipelines(options):
    ...
    if not fJSON:
        exit(
            "No logpipelines are locally available. Consider pulling logpipelines first."
        )
    ...
    for item in fJSON:
        with open(item) as f:
            ...
            if not arguments["--dry-run"]:
                r = requests.post(
                    "{}api/v1/logs/config/pipelines".format(options["api_host"]),
                    headers=headers,
                    json=data,
                )
                json_data = json.loads(r.text)
...
def push_notebooks(options):
    ...
    for notebook in notebooks:
        with open(notebook) as f:
            ...
            if not arguments["--dry-run"]:
                r = requests.post(
                    "{}api/v1/notebook?api_key={}&application_key={}".format(
                        options["api_host"]
                    ),
                    headers=headers,
                    json=data,
...
def push_slos(options):
    ...
    for slo in slos:
        with open(slo) as f:
            ...
            if not arguments["--dry-run"]:
                r = requests.post(
                    "{}api/v1/slo".format(options["api_host"]),
                    headers=headers,
                    json=data,
                )
...
if __name__ == "__main__":
    ...
    if arguments["--dry-run"]:
        print(
            "You are running in dry-mode. No changes will be commmited to your Datadog account(s)."
        )
